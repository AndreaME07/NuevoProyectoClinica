...
class ModelUser:
    ...
    def login(self, rfc, mysql):
        try:
            ...
            sql = """SELECT id, RFC, pass, nombres, apeP, apeM, cedula, correo, id_rol FROM medicos WHERE RFC=%s"""
    ...
    def get_by_id(self, mysql, id):
        try:
            cursor = mysql.connection.cursor()
            sql = "SELECT id, RFC, pass, nombres, apeP, apeM, cedula, correo, id_rol FROM medicos WHERE id=%s"
